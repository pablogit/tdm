...
# -*- coding: utf-8 -*-
"""
Version  :  V2.5 adapted to modify labs files (CSV or XML) to be imprted in Nicer Stat
Author   : 	Pablo Iriarte, CHUV/IUMSP 2016. pablo.irairte@chuv.ch, UNIGE 2017. pablo.irairte@unige.ch
...
at_version = "V2.5 simplified ready for Nicer Stat with CSV and XML export and also for simple sort of PDFs (2019-02-21)"
...
mypass = '48ksdq34pthfd9ruwk381'

# Define the original place for the dialog window selecting files and folders
myinitialdir = 'C:\my_folder'

# Define help texts diplayed in dialog window selecting files and folders
mydialoglabfile = 'Choisir le fichier CSV ou XML du labo'
mydialogpdffolder = 'Choisir le dossier contenant les fichiers PDF'
filetype_false = 'Le fichier Nicer doit être en format CSV or XML'
...
root = Tkinter.Tk()
# getting term list
terms_file = tkFileDialog.askopenfile(parent=root, initialdir=myinitialdir, title='Choisir la liste des termes', filetypes =(('TXT file' , '*.txt'), ('all files' , '*.*')))
# if file1 != None:
...
if donicer == 1:
	# getting nicer file
	mynicerfile = tkFileDialog.askopenfilename(parent=root, initialdir=myinitialdir, title=mydialoglabfile, filetypes =(('CSV or XML files' , '*.csv;*.xml'), ('all files' , '*.*')))
	mydir = os.path.split(mynicerfile)[0]
	myfileorig = os.path.split(mynicerfile)[1]
	myfileext = myfileorig.split('.')[-1]

	# getting files to be screened
	mydirin = tkFileDialog.askdirectory(parent=root, initialdir=mydir, title=mydialogpdffolder)
	os.chdir(mydirin)
	fnames = os.listdir(os.getcwd())
	
	# create SCAN FOLDER
	newpath_scan = mydir + '/_SCANPDFS/'
	if not os.path.exists(newpath_scan):
		os.makedirs(newpath_scan)
	mydir = newpath_scan

	# Method depending on filetype : CSV or XML
	# Parse Nicer CSV and extract PDFs files names
	if ((myfileext == "csv") | (myfileext == "CSV")):
		nicerpdfs = [ ]
		p = re.compile(r'[^;,\t]+\.pdf')
		with open(mynicerfile) as infile:
			for line in infile:
				m = re.findall(p, line)
				if m:
						# print (m[0])
						nicerpdfs.append(m[0])
		# sys.exit()
	elif ((myfileext == "xml") | (myfileext == "XML")):
		# https://docs.python.org/2/library/xml.etree.elementtree.html
		nicerpdfs = [ ]
		mytree = ET.parse(mynicerfile)
		myroot = mytree.getroot()
		for Einsendung in myroot.findall('Einsendung'):
			for Visit in Einsendung.findall('Visit'):
				VisitBefund = Visit.find('VisitBefund').text
				nicerpdfs.append(VisitBefund)
	else:
		print (filetype_false)


if donicer != 1:
	# getting files to be screened
	mydirin = tkFileDialog.askdirectory(parent=root, initialdir=myinitialdir, title=mydialogpdffolder)
	os.chdir(mydirin)
	fnames = os.listdir(os.getcwd())
	...
	# create SCAN FOLDER
	newpath_scan = mydirin + '/_SCANPDFS/'
	if not os.path.exists(newpath_scan):
		os.makedirs(newpath_scan)
	mydir = newpath_scan

print '**********************************************'
print 'Version: ' + at_version
print '**********************************************'
print 'Folder creation - starting'

# création des dossiers pour enregistrer les fichiers résultants
newpath_ori = mydir + '/ORIGINAUX/'
...
# create a folder to store encrypted PDF or origianls PDF
if docopyorig == 1:
	if not os.path.exists(newpath_ori):
...
print 'Destination folder: ' + mydir
...
if donames == 1:
	...
	labo_dafault = ""
	timestr = time.strftime("%Y%m%d-%H%M%S")
	# loop in PDFs cited in CSV or XML only, not in all PDFs in folder
	# myfilesnb = len(fnames)
	if donicer == 1:
                myfilesnb = len(nicerpdfs)
        else:
                myfilesnb = len(fnames)
	myfilesnb = str(myfilesnb)
	myfilesnb_i = 1
	print 'Searching terms in ' + myfilesnb + ' files - starting'
	...
	for fname in fnames:
				# limiter la recherche aux fichiers cités dans le fichier pour Niver
				if donicer != 1:
                                        nicerpdfs = fnames
				if fname in nicerpdfs:
						print 'File ' + str(myfilesnb_i) + ' of ' + myfilesnb + ' - ' + fname + ' - starting '
						if dorename == 1:
								new_fname = ''.join(c for c in fname if c in string.printable)
								new_fname = new_fname.replace('.pdf', '_' + timestr + '.pdf')
						...
						if fname.endswith('.pdf'):
								...
								if data:
										...
										# noms_result['name'] = myname
										# noms_result['filename2'] = myfile
										# oms_results.append(noms_result)
										# Ecrire le fichier convertit en txt (enlever les commentaires pour obtenir les fichiers)
										# dst = newpath_txt + '/' + new_fname + '.txt'
										# oa = open(dst, 'w')
										# oa.write(data)
										# oa.close()
										result['_original_filename'] = fname
										result['_total'] = total
										result['_ntotal'] = ntotal
										# result['labo'] = 'NA'
										# result['name'] = 'NA'
										...
										# Copie du fichier dans le dossier correspondant
										if total > 0:
												dst = newpath_pos + '/' + myfile
												with file(dst, "wb") as outfp1:
														writer.write(outfp1)
												# copyfile(fname, dst)
										if total == 0:
												dst = newpath_neg + '/' + myfile
												with file(dst, "wb") as outfp2:
														writer.write(outfp2)
												# copyfile(fname, dst)
										print 'File ' + str(myfilesnb_i) + ' of ' + myfilesnb + ' - ' + fname + ' - done '
...
print 'Searching terms in ' + myfilesnb + ' files - done'
...
print 'Saving results for each term in a CSV file - done'
...
if donames == 1:
		...
		for nom in noms_results:
				...
				outF.write('\n')
		outF.close()
		print 'Saving results for names in a CSV file - done'
...
if donicer == 1:
		print 'Saving files for Nicer Stat - starting'
		p2 = re.compile(r'[^;,\t]+\.pdf')
		nicerpdfsill = [ ]
		if (myfileext == "csv"):
			with  open (mynicerfile ,  'rb')  as  csvfile :
					myfilenicer = os.path.split(mynicerfile)[1]
					dst_pos = newpath_res + '/positifs_utf8_' + timestr + '_' + myfilenicer
					dst_pos2 = newpath_pos + '/positifs_utf8_' + timestr + '_' + myfilenicer
					dst_neg = newpath_res + '/negatifs_' + timestr + '_' + myfilenicer
					dst_ill = newpath_res + '/introuvables_' + timestr + '_' + myfilenicer
					dst_pos_ansi = newpath_res + '/positifs_ansi_' + timestr + '_' + myfilenicer
					dst_pos_ansi2 = newpath_pos + '/positifs_ansi_' + timestr + '_' + myfilenicer
					outF_pos = open(dst_pos, 'w')
					outF_neg = open(dst_neg, 'w')
					outF_ill = open(dst_ill, 'w')
					linecount = 1
					for linenicer in csvfile:
							line2 = linenicer.strip()
							# line2 = line2.encode('latin-1')
							if linecount == 1:
									outF_pos.write(line2)
									outF_neg.write(line2)
									outF_ill.write(line2)
									outF_pos.write('\n')
									outF_neg.write('\n')
									outF_ill.write('\n')
							linecount = linecount + 1
							m2 = re.findall(p2, line2)
							if m2:
									# print (m2[0])
									for result in results:
											myfilenicername = result['_original_filename']
											myfilenicertotal = result['_total']
											# print (myfilenicername)
											if myfilenicername == m2[0]:
													if myfilenicertotal > 0:
															outF_pos.write(line2)
															outF_pos.write('\n')
													if myfilenicertotal == 0:
															outF_neg.write(line2)
													...
													if m2[0] not in nicerpdfsill:
															nicerpdfsill.append(m2[0])
															outF_ill.write(line2)
															outF_ill.write('\n')
					outF_pos.close()
					outF_neg.close()
					outF_ill.close()
			with io.open(dst_pos, mode='r', encoding='utf-8') as fd:
				content = fd.read()
				with io.open(dst_pos_ansi, mode='w', encoding='latin-1', errors='ignore') as fd:
					fd.write(content)
			copyfile(dst_pos, dst_pos2)
			copyfile(dst_pos_ansi, dst_pos_ansi2)
			print 'Saving files for Nicer Stat - done'
		elif (myfileext == "xml"):
			# https://docs.python.org/2/library/xml.etree.elementtree.html
			myfilenicer = os.path.split(mynicerfile)[1]
			dst_pos = newpath_res + '/positifs_' + timestr + '_' + myfilenicer
			dst_pos2 = newpath_pos + '/positifs_' + timestr + '_' + myfilenicer
			dst_neg = newpath_res + '/negatifs_' + timestr + '_' + myfilenicer
			dst_ill = newpath_res + '/introuvables_' + timestr + '_' + myfilenicer
			outF_pos = open(dst_pos, 'w')
			outF_neg = open(dst_neg, 'w')
			...
			# positifs
			mytreepos = ET.parse(mynicerfile)
			myrootpos = mytreepos.getroot()
			for Einsendung in myrootpos.findall('Einsendung'):
				for Visit in Einsendung.findall('Visit'):
					VisitBefund = Visit.find('VisitBefund').text
					# nicerpdfs.append(VisitBefund)
					for result in results:
						...
						myfilenicertotal = result['_total']
						# print (myfilenicername)
						if myfilenicername == VisitBefund:
							if myfilenicertotal == 0:
								# remove visit node in positif xml file
								# Einsendung.remove(Visit)
								myrootpos.remove(Einsendung)
			treepos = ET.ElementTree(myrootpos)
			treepos.write(outF_pos, encoding='utf-8')

			# negatifs
			mytreeneg = ET.parse(mynicerfile)
			myrootneg = mytreeneg.getroot()
			for Einsendung in myrootneg.findall('Einsendung'):
				for Visit in Einsendung.findall('Visit'):
					VisitBefund = Visit.find('VisitBefund').text
					# nicerpdfs.append(VisitBefund)
					for result in results:
						...
						if myfilenicername == VisitBefund:
							if myfilenicertotal > 0:
								# remove visit node in negatif xml file
								# Einsendung.remove(Visit)
								myrootneg.remove(Einsendung)
			treeneg = ET.ElementTree(myrootneg)
			treeneg.write(outF_neg, encoding='utf-8')
			
			# introuvables
			mytreeill = ET.parse(mynicerfile)
			myrootill = mytreeill.getroot()
			for Einsendung in myrootill.findall('Einsendung'):
				for Visit in Einsendung.findall('Visit'):
					VisitBefund = Visit.find('VisitBefund').text
					# nicerpdfs.append(VisitBefund)
					for result in results:
						...
						# print (myfilenicername)
						if myfilenicername == VisitBefund:
							# remove visit node in introuvables xml file
							# Einsendung.remove(Visit)
							myrootill.remove(Einsendung)
			treeill = ET.ElementTree(myrootill)
			treeill.write(outF_ill, encoding='utf-8')
